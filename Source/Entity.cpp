...
void Entity::setInitX(const int& x) { this->x = x; this->init_x = x; }
void Entity::setInitY(const int& y) { this->y = y; this->init_y = y; }
...
void Entity::moveRight(char map[16][28], const bool& red)//il bool riguarda il nemico e se quest'utlimo ha il cappello rosso, cioè ha raccolto una moneta
{
        ...
        && (map[(getY() - 18) / 20][(getX() / 20) + 1] != '@' && map[getY() / 20][(getX() / 20) + 1] != '@'))
    {
        ...
        if (map[(getY() - 18) / 20][getX() / 20] == '-' && map[((getY()) / 20)][(getX() / 20)] == ' ' 
            && map[((getY() - 5) / 20)][(getX() / 20)] == ' ') //se si sta avvicinando ad una corda, ma non è all'altezza giusta cade
        {
            red ? setFrame(11) : setFrame(4);
            setFall(true);
        }
        if (map[(getY() - 18) / 20][getX() / 20] == '-' && map[((getY()) / 20)][(getX() / 20)] == ' ' 
            && map[((getY() - 5) / 20)][(getX() / 20)] == '-') //è all'altezza giusta, si aggrappa
            setY(getY() - 5);
        setMirrorX(false);
        setMirrorRope(false);
    }
    if ((map[(getY() + 5) / 20][(getX() / 20) + 1] == ' ' || map[(getY() + 5) / 20][((getX() + 10) / 20)] == ' ' 
        || map[(getY()+5)/20][(getX()+10)/20] == 'S') && map[(getY() - 18) / 20][(getX() / 20) + 1] != '-' 
...
void Entity::moveLeft(char map[16][28], const bool& red)//stessa logica del moveRight, uniche differenze nelle operazioni con i pixel
{
    ...
    if ((map[(getY() + 5) / 20][(getX() + 18) / 20] == ' ' || map[(getY() + 5) / 20][(getX() + 18) / 20] == '$' || map[(getY()+5)/20][(getX()+18)/20] == 'S' ) // aggiunto S
        ...
        && map[(getY() + 5) / 20][(getX() / 20)] != 'H' && map[(getY() + 5) / 20][(getX() / 20)] != '#'
        && map[(getY() + 5) / 20][(getX() / 20)] != '@' && map[(getY() + 5) / 20][(getX() / 20)] != '}')
    {
        red ? setFrame(11) : setFrame(4);
        setFall(true);
