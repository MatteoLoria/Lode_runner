...
void Entity::moveRight(char map[16][28]){
    if((map[(getY()-18)/20][(getX()/20)+1] != '#' && map[getY()/20][(getX()/20)+1] != '#')//controllo blocchi
        && (map[(getY()-18)/20][(getX()/20)+1] != '@' && map[getY()/20][(getX()/20)+1] != '@')){//TODO: da testare
        setX(getX()+5);
        ...
        if(map[(getY()-18)/20][getX()/20] == '-' && map[((getY())/20)][(getX()/20)] == ' ' 
            && map[((getY()-5)/20)][(getX()/20)] == ' ')//troppo lontano dalla corda. Cadi
        {
            setFrame(4);
            setFall(true);
        }
        if(map[(getY()-18)/20][getX()/20] == '-' && map[((getY())/20)][(getX()/20)] == ' ' 
            && map[((getY()-5)/20)][(getX()/20)] == '-')//sei vicino alla corda. Aggrappati
            setY(getY()-5);
        setMirrorX(false);
        setMirrorRope(false);
    }
    if((map[(getY()+5)/20][(getX()/20)+1] == ' ' || map[(getY()+5)/20][((getX()+10)/20)] == ' ')  && map[(getY()-18)/20][(getX()/20)+1] != '-' && map[(getY()-18)/20][(getX()/20)+1] != 'S' 
        && map[(getY()-18)/20][(getX()/20)+1] != 'H' && map[(getY()+5)/20][(getX()/20)] != 'H' 
...
void Entity::moveLeft(char map[16][28]){
    if(map[(getY()-18)/20][(getX()-1)/20] != '#' && map[getY()/20][(getX()-1)/20] != '#'){//controllo blocchi
        setX(getX()-5);
    ...
    if(map[(getY()+5)/20][(getX()+18)/20] == ' ' && map[(getY()-18)/20][(getX()-1)/20] != '-' 
        && map[(getY()-18)/20][(getX()-1)/20] != 'H' && map[(getY())/20][(getX()/20)] != 'H' 
        && map[(getY()+5)/20][(getX()/20)] != '#' && map[(getY()+5)/20][(getX()/20)] != '@'){
        setFrame(4);
        setFall(true);
    }
    ...
    if(lastIsLeft){
        if(map[getY()/20][(getX()+10)/20]=='H') //moving from right and the stair is at left
        {
            ...
            if(getMirrorY())
                ...
                setMirrorY(true);
        }else if(map[getY()/20][(getX()+5)/20]=='H'){ //moving from right and the stair is at right
            setX(((getX()+5)/20)*20);
            setY(getY()-5);
            setFrame(3);
            if(getMirrorY())
        ...
        if(map[getY()/20][(getX()+10)/20]=='H') //moving from left and the stair is at right
        {
            ...
            if(getMirrorY())
                ...
                setMirrorY(true);
        }else if(map[getY()/20][(getX()+15)/20]=='H'){ //moving from left and the stair is at left
            setX(((getX()+15)/20)*20);
            setY(getY()-5);
            setFrame(3);
            if(getMirrorY())
